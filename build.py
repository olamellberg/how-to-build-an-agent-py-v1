...
def read_markdown(path: str) -> str:
    """Read markdown file and normalize line endings"""
    with open(path, 'r', encoding='utf-8') as f:
        content = f.read()
    # Normalize CRLF to LF (Windows line endings break fenced code blocks)
    return content.replace('\r\n', '\n').replace('\r', '\n')


def strip_leading_title(md_content: str) -> str:
    """
    Strip leading H1 and subtitle (H3 or italic) from markdown content.
    These are duplicated by the build script's generated article header.
    """
    ...
    # Skip H1 (# Title)
    if start_index < len(lines) and lines[start_index].startswith('# '):
        start_index += 1
    ...
    # Skip subtitle: H3 (### Subtitle) or italic (*Subtitle*)
...
def extract_h2_sections(md_content: str) -> List[Tuple[str, str, str]]:
    ...
    Ignores H2 headers inside fenced code blocks.
    """
    sections = []
    lines = md_content.split('\n')
    current_section = []
    ...
    for line in lines:
        ...
        # Only treat as section header if not inside a code block
...
def generate_article_html(article: Article, sv_content: str, en_content: str, all_articles: List[Article], categories: Dict) -> str:
    ...
    # Strip leading H1/subtitle from markdown (these are generated by the build script)
    sv_content = strip_leading_title(sv_content)
    en_content = strip_leading_title(en_content)
